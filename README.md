...
- **事件通知推送**（可选）：HTTP(S) POST + Bearer 鉴权，**系统事件**（宕机 / 强制关机 / 看门狗停止）与**网页电源操作**（开机 · 软关机 · 重启 · 强制关机 · 设备重启）均可推送；「服务器电源控制」卡片含「网页操作发送通知」开关控制后者，另有「USB 串口断开触发重启」开关控制是否因 USB 断开而自复位看门狗设备。
...
| **GPIO8** | 输出 | `CONFIG_LED_GPIO_PIN` | **状态 LED** | 高电平点亮；慢闪 1 Hz / 快闪 5 Hz / OTA 中 200 ms |
...
- 串联 **330 Ω ~ 1 kΩ** 电阻接 LED 正极到 GPIO8（LED 负极接地）；若 LED 为高亮/大功率，改用三极管驱动
...
├── CMakeLists.txt           # 项目注册 (PROJECT_VER = 1.5.0)
...
**版本号**：根 `CMakeLists.txt` 中的 `set(PROJECT_VER "1.5.0")` 控制。发版时改这一处即可，
版本号与构建日期会自动写入镜像 `esp_app_desc_t`，Web 控制台页头显示
`版本 v1.5.0 · 编译日期 <构建当日>`，OTA 上传页解析的也是同一字段。
...
- **服务器电源控制**：开机 / 软关机 / 重启 / 强制关机 / 重置网络；卡片内含「网页操作发送通知」开关（开启后网页电源操作也会推送通知），以及「USB 串口断开触发重启」开关（开启后 USB 串口一旦断开即软重启看门狗设备）
...
```
{{VERSION}} {{BUILD_DATE}} {{LED_CLASS}} {{STATE_TEXT}} {{USB_STATE}} {{WEB_NOTIFY_STATE}}
