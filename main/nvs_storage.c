...
esp_err_t nvs_save_web_notify(bool enabled)
{
    if (!s_initialized) return ESP_FAIL;

    esp_err_t ret = nvs_set_u8(s_nvs_handle, NVS_KEY_WEB_NOTIFY, enabled ? 1 : 0);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to save web notify flag: %s", esp_err_to_name(ret));
        return ret;
    }
    ESP_LOGI(TAG, "Web action notify: %s", enabled ? "enabled" : "disabled");
    return nvs_commit(s_nvs_handle);
}
...
esp_err_t nvs_load_web_notify(bool *enabled)
{
    ...
    uint8_t v = 1;   // 默认开启: 网页触发的电源操作也发通知 (需通知总开关启用)
