...
// 状态 LED 根据"网络连通 / OTA 升级"切换 (GPIO8, 高电平点亮):
//   - OTA 写入中:       200ms 间隔快闪 (LED_BLINK_OTA)
//   - 已连上 WiFi (STA): 1Hz 慢闪      (LED_BLINK_SLOW)
//   - 未联网 / 配网中:   5Hz 快闪      (LED_BLINK_FAST)
// 该函数只设置"期望状态", 真正的电平翻转由 gpio_led_update() 完成。
// (看门狗相关的 LED 指示已统一收口到这里, 不再由 watchdog.c 各自设置,
//  以免多个模块抢夺同一个物理 LED 导致行为混乱。)
static void update_status_led(void)
{
    // OTA 升级中: 最高优先级, 200ms 间隔快闪
    if (ota_get_state() == OTA_STATE_WRITING) {
        gpio_set_led_state(LED_BLINK_OTA);
        return;
    }
    // 已拿到 STA IP = 联网成功 -> 慢闪 1Hz; 否则 (配网/连接中/断线) -> 快闪 5Hz
    bool wifi_connected = (xEventGroupGetBits(s_wifi_event_group) & WIFI_CONNECTED_BIT) != 0;
    gpio_set_led_state(wifi_connected ? LED_BLINK_SLOW : LED_BLINK_FAST);
}

// 独立 LED 任务: 与系统状态机解耦。
// 之前 LED 刷新放在 system_task 主循环里, 而主循环在配网 (SmartConfig 阻塞等待)
// 或 WiFi 连接等待 (单次阻塞 15s) 时会长时间不调用 gpio_led_update(),
// 导致 LED 被"冻结"在某一个电平 —— 表现为"通电闪一下就再也不亮"。
// 这里用独立任务以 50ms 周期持续刷新, 保证任何状态下 LED 都按
// 网络/OTA 状态稳定闪烁。
...
static void system_task(void *pvParameter)
{
    ...
    while (1) {
        // LED 刷新已交给独立的 led_task (与系统状态机解耦),
        // 这样即使主循环在配网/连网等待中长时间阻塞, LED 也能持续闪烁。
...
void app_main(void)
{
    ...
    // 启动独立 LED 任务 (与系统状态机解耦, 持续刷新 LED 闪烁)
